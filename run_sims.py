...
@dataclass
class SimPair:
    project : str        # e.g. "Detector_v1.0"
    dut_path : Path      # full path to filename.v
    tb_path  : Path      # full path to filename_tb.v
    dut_name : str       # e.g. "detector"
...
def discover_pairs(root: Path) -> list[SimPair]:
    """
    Walk from root, find every folder named IMPL_FOLDER_NAME,
    pair each <name>.v with <name>_tb.v inside it.
    Groups under the immediate parent of the Verilog-Implementation folder.
    """
    pairs: list[SimPair] = []
    ...
    for impl_dir in sorted(root.rglob(IMPL_FOLDER_NAME)):
        ...
        # Immediate parent is the "project" name
        project_name = impl_dir.parent.name

        # Collect all non-testbench .v files
        src_files = [
        ...
        for src in sorted(src_files):
            dut_name = src.stem                         # e.g. "detector"
...
def setup_windows(missing: list[str]) -> bool:
    ...
    pkg_map = {"iverilog": "IcarusVerilog.IcarusVerilog",
...
def ensure_tools() -> bool:
    """Check iverilog + gtkwave; install if missing. Returns True if both available."""
    ...
    if answer in ("", "y", "yes"):
        ...
        if os == "Linux":
            ...
            if _tool_installed("brew"):
                ...
                _run(["brew", "install"] + missing, check=False)
...
def run_simulation(pair: SimPair, open_wave: bool = True) -> bool:
    """Compile and run a single simulation pair. Returns True on success."""
    with tempfile.TemporaryDirectory() as tmpdir:
        out_bin  = Path(tmpdir) / "sim.out"
        vcd_file = Path(tmpdir) / "dump.vcd"
        ...
        # ── Simulate ─────────────────────────────
        print(cyan(f"  Running simulation …\n"))
        sim_env = os.environ.copy()
        sim_env["VCD_FILE"] = str(vcd_file)   # testbench can use $dumpfile env if desired
        ...
        if sim_res.returncode != 0:
            print(red("  ✗ Simulation exited with errors."))
            return False

        # ── Waveform ─────────────────────────────
        # Look for any .vcd the testbench dumped
        vcd_files = list(Path(tmpdir).glob("*.vcd"))
        if vcd_files and open_wave:
            vcd = vcd_files[0]
            print(cyan(f"\n  Opening waveform: {vcd.name}"))
            # Non-blocking so user can continue the menu
            subprocess.Popen(
                ["gtkwave", str(vcd)],
            ...
            print(dim("  (No .vcd file produced — add $dumpfile/$dumpvars to testbench for waveforms)"))
...
def print_menu(pairs: list[SimPair], current_project: Optional[str] = None):
    print()
    last_project = None
...
def interactive_loop(pairs: list[SimPair], root: Path):
    while True:
        ...
        if raw in ("q", "quit", "exit", ""):
            ...
            for p in pairs:
                print(bold(f"\n  ── {p.label()} ──"))
                ok = run_simulation(p, open_wave=False)   # no wave popup for bulk runs
...
def main():
    ...
    # Script lives anywhere — always search from its own directory upward to repo root
    # Strategy: walk up until we find a folder that contains IMPL_FOLDER_NAME somewhere,
    # or just use the script's directory as the root.
    ...
    if not pairs:
        ...
        print(red(f"  Expected structure:  <project>/{IMPL_FOLDER_NAME}/<name>.v + <name>_tb.v"))
        sys.exit(1)
